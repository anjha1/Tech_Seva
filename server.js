...
const userSchema = new mongoose.Schema({
    ...
    address: { type: String }, // General user address
    city: { type: String }, // General user city
    ...
    workingLocation: { // For technicians' service area
        ...
        city: { type: String },
        pincode: { type: String },
        latitude: { type: Number }, // Changed to latitude
        longitude: { type: Number } // Changed to longitude
    },
...
const jobSchema = new mongoose.Schema({
    ...
    customerName: String,
    customerEmail: String, // Added for convenience
    customerPhoneNumber: String, // Added for convenience
    applianceType: { type: String, required: true }, // e.g., AC Repair, Plumbing
    problemDescription: String,
    location: {
        address: String,
        city: String,
        pincode: String, // If precise pincode is given
        latitude: Number,
        longitude: Number
    },
    ...
    faultyParts: [String], // Diagnosed faulty parts by technician
    ...
    quotation: {
        ...
        createdAt: Date // When quotation was issued
    },
    payment: {
        amount: Number,
        method: String, // e.g., 'UPI', 'Cash', 'Card'
        details: Object, // Transaction details from payment gateway
        status: { type: String, enum: ['Pending', 'Paid', 'Failed', 'Refunded'], default: 'Pending' },
        paidAt: Date,
        transactionId: { type: String, unique: true, sparse: true } // Unique ID for each payment transaction
    },
    completedAt: Date, // When job was actually completed
    rating: { type: Number, min: 1, max: 5 }, // Customer rating
    reviewText: String, // Customer review text
    reviewedAt: Date, // When review was submitted
    proofImages: [String], // Array of image URLs/Base64 for job completion/diagnosis proof
    
    // For job assignment logic (e.g., technicians proposing to take jobs) - not fully implemented in this server.js
    technicianProposals: [{
...
app.post('/api/user/profile/update', authenticateToken, authorizeRoles(['user']), async (req, res) => {
    ...
    try {
        ...
        const { fullName, phoneNumber: rawPhoneNumber, address, city, latitude, longitude } = req.body;
        ...
        } else if (phoneNumber === undefined && user.phoneNumber) { // If user explicitly cleared phone number
        ...
        // Update session user data to reflect changes immediately on the frontend
        // Note: req.session.user might not contain all fields of req.user from DB
        req.session.user.fullName = user.fullName;
        req.session.user.phoneNumber = user.phoneNumber; 

        // Re-fetch the user to ensure the latest data is sent back (including computed/defaulted fields)
...
app.post('/api/book-service', isAuthenticated, isUser, async (req, res) => {
    ...
    try {
        const { applianceType, location, scheduledDateTime, notes, latitude, longitude } = req.body; 
        ...
        if ((latitude && isNaN(parseFloat(latitude))) || (longitude && isNaN(parseFloat(longitude)))) {
            console.warn('[BOOK SERVICE API] Invalid latitude or longitude provided:', { latitude, longitude });
        ...
        const availableTechnicians = await User.find({ role: 'technician', kycStatus: 'approved', status: 'active' }); // Assuming 'status' field for active technicians
        ...
        const newJobId = `J${Date.now().toString().slice(-6)}${Math.floor(Math.random() * 100).toString().padStart(2, '0')}`; // Ensures 2 digits
        ...
        console.log('[BOOK SERVICE API] Attempting to save new job to DB:', newJob.jobId);
        await newJob.save();
...
app.post('/api/reverse-geocode', async (req, res) => {
    ...
    try {
        ...
        if (data.status === 'OK' && data.results && data.results.length > 0) {
            ...
            res.json({ success: true, address: formattedAddress });
        } else {
